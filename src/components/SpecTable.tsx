          ...
          <tbody>
            {spec.rows.map((row, idx) => (
              <tr key={idx} className={idx % 2 === 0 ? 'bg-slate-800/50' : 'bg-slate-800/20'}>
                <td className="px-4 py-2.5 text-slate-300 font-medium w-1/2 align-top">{row.label}</td>
                <td className="px-4 py-2.5 text-white font-mono">{row.value}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-brand-500/10">
              {table.headers.map((header, idx) => (
                <th
                  key={idx}
                  className={`px-4 py-3 font-semibold text-brand-300 ${idx === 0 ? 'text-left' : 'text-center'}`}
                >
                  {header}
                </th>
              ))}
          ...
          <tbody>
            {table.rows.map((row, idx) => (
              <tr key={idx} className={idx % 2 === 0 ? 'bg-slate-800/50' : 'bg-slate-800/20'}>
                <td className="px-4 py-3 font-mono font-bold text-white whitespace-nowrap">{row.label}</td>
                {row.values.map((value, vIdx) => (
                  <td key={vIdx} className="px-4 py-3 text-slate-300 text-center">
                    {value}
                  </td>
                ))}
              </tr>
